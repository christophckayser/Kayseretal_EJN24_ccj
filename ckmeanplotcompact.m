function h = ckmeanplotcompact(data,errortype,showunit,connectdata,Colorvector,x,symvec)
...
if showunit
  for k=1:n
    ...
    plot(xpos,data(:,k),symvec(k),'Color',Colorvector(k,:));
    keepX(:,k) = xpos;
  end
end
% connect lines
...
for k=1:n
  % mean
  plot(x(k),Mval(k),'.','color',Colorvector(k,:),'MarkerSize',26);
  % sem
  h = line([x(k) x(k)],[Eval(1,k) Eval(2,k)]);
  set(h,'LineWidth',2,'color',Colorvector(k,:));
  plot(x(k),Mval(k),'.','color',[0 0 0],'MarkerSize',12);
